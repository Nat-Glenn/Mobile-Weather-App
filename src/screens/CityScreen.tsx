  ...
  return (
    <AppShell>
      <>
        ...
        {/* Bottom nav: Settings + Home (no + button) */}
...
function HourlyAndWidgets({
  ...
  const iconColor = theme === 'light' ? '#111111' : '#ffffff';
          ...
          return (
            ...
            >
              ...
              <View style={styles.hourIconWrapper}>
                {getWeatherIcon(point.weatherCode, 26, iconColor)}
              </View>
        ...
        return (
          <View key={day.date} style={styles.weeklyCard}>
            ...
            <View style={styles.weekIconWrapper}>
              {getWeatherIcon(day.weatherCode, 24, iconColor)}
            </View>
          </View>
        );
      })}
...
function getWeatherIcon(code: number, size: number, color: string) {
  if (code === 0) {
    return (
  ...
  if (code === 45 || code === 48) {
    return (
      <MaterialCommunityIcons name="weather-fog" size={size} color={color} />
    );
  }
...
function createStyles(theme: ThemeName) {
  ...
  return StyleSheet.create({
    ...
    hourIconWrapper: { marginVertical: 4 },
    hourTemp: {
      fontSize: 13,
      fontWeight: '600',
